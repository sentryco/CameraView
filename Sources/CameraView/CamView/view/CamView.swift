 ...
 * - Description: The CamView class is a custom UIView that utilizes the device's camera to capture and process QR codes. It provides a simple interface for starting and stopping the camera, and delivers the captured QR code data through a callback function. This class is designed to be easy to integrate into any iOS application that requires QR code scanning functionality.
...
public final class CamView: UIView, AVCaptureMetadataOutputObjectsDelegate {
    ...
    * - Description: This initializer is used to create a new instance of CamView. It sets up the camera view with the specified frame size and sets the callback function to be called when a QR code is captured. The callback function receives the payload of the captured QR code as its parameter.
    * - Parameters:
    *   - frame: The size of the camera view
    *   - onComplete: The callback for when a QR code is captured
    * - Note: This function sets the onComplete callback to the specified callback, calls the designated initializer of the superclass with the specified frame, creates the capture session, initializes the video preview layer and adds it as a sublayer to the view's layer, creates the QR code frame view, and sets the accessibility identifier of the view for UI testing. In a real world application, you could add support for customizing the appearance and behavior of the view, and handling errors and user input.
